...
const DeckCard = () => {
    const {currentPlayer, you:{id}, gameState, tally} = GameStore.useState(s => s);
    ...
    return gameState === "RUNNING" ? (
        <div className="deck-card">
            ...
            {isInTurn ? <Button onClick={draw} className="draw-button" variant="light">{!tally ? <>Draw</> : <>Take {tally}</>}</Button> : null}
        </div>
    ) : null
}
